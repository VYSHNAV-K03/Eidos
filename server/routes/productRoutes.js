...
router.get("/products", authMiddleware, getProductsSupplier);
router.get("/productscustomer", authMiddleware, getProducts);
...
router.put("/products/:id/update-stock", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Stock updated successfully", product: updatedProduct });
  } catch (error) {
    res
      .status(500)
